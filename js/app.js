...
post_details.onclick = (e) => {
    ...
    if(e.target.classList.contains('edit-btn')){
        ...
        edit_post_form.innerHTML = `
            ...
            <div class="my-3">
                <input type="text" class="form-control" name="postID" value="${postID}">
            </div>
            <div class="my-3">
...
//    let newPostId = all_data[0].postID;

//    let editedPost = all_data.find(data => data.postID == postID );
...
// console.log(all_data);
   ...
   all_data.map((data, index) => {
    ...
    // let info = data.filter(d => d.postID === postID);

    if(data.postID == postID){
        
        data.authName = authName
        data.authPhoto = authPhoto
        data.postPhoto = postPhoto
